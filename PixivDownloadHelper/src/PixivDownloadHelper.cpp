    ...
    : QWidget(parent), opacity(1.0)
{
    ...
    //信号槽实现滑块改变背景透明度
    connect(settingWidget, &SettingWidget::changeTransparencySliderValueChange,
        this, &PixivDownloadHelper::changeTransparency);
    //信号槽实现改变背景图片
    connect(settingWidget, &SettingWidget::backgroundImgChanged,
    ...
    connect(stackedWidget, &StackedWidget::enterSignal,
        menuWidget, &MenuWidget::widgetFold);
