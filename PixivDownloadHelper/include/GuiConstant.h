...
inline std::string getSheetColor(const QColor& color) {
    return std::string("rgba(") + std::to_string(color.red()) + std::string(",") + std::to_string(color.green()) +
           std::string(",") + std::to_string(color.blue()) + std::string(",") + std::to_string(color.alpha()) +
           std::string(")");
}
...
constexpr const char* CONFIG_UNDONE_PATH = "./config/downloadData.cfg";                    // 保存未完成的下载项目信息
...
// 正则匹配规则表达式
// pixiv单个作品中的图片  url匹配规则
constexpr const char* REGEX_PIXIV_ILLUST = "https://i.pximg.net/img-original[^\"]+";
// pixiv单个作品  url匹配规则
...
// telegram匹配规则
constexpr const char* REGEX_TELEGRAM = "https://telegra.ph/[\\S]+";
...
constexpr size_t PIXIV_DOWNLOAD_ITEM_WITHOUT_PIC_HEIGHT{
    PIXIV_DOWNLOAD_TITLE_WIDGET_HEIGHT + PIXIV_DOWNLOAD_STATE_HEIGHT + 3 * MARGIN_WIDTH}; // pixiv下载项目无缩略图高度
...
constexpr size_t PIXIV_DOWNLOAD_ITEM_WITH_PIC_HEIGHT{PIXIV_DOWNLOAD_ITEM_WITHOUT_PIC_HEIGHT +
                                                     PIXIV_PREVIEW_SIZE.height() +
                                                     2 * MARGIN_WIDTH}; // pixiv单个下载项目有缩略图高度
